...
func Create(Array *ArrayGlobal, input *Tim) {
	var (
		...
		skor,jmltim, jmluniv, jmlskor, jmlLomba, jenis int
	...
	for status == true {
		...
		if jenis == 1 {
			...
			arrLomba.Datamining[jmlLomba].Nama = namatim
			jmltim += 1
			JumlahTim += 1
			fmt.Println("masukkan asal universitas : ")
			fmt.Scan(&namauniv)
			Array[jmluniv].Univ = namauniv
			arrLomba.Datamining[jmlLomba].Univ = namauniv
			jmluniv += 1
			fmt.Println("masukkan nilai skor : ")
			fmt.Scan(&skor)
			Array[jmlskor].Skor = skor
			arrLomba.Datamining[jmlLomba].Skor = skor
			jmlskor += 1
			jmlLomba++
			jmlJenis1++
		}
		if jenis == 2 {
			...
			fmt.Scan(&namatim)
			Array[jmltim].Nama = namatim
			arrLomba.Pemrograman[jmlLomba].Nama = namatim
			jmltim += 1
			JumlahTim += 1
			fmt.Println("masukkan asal universitas : ")
			fmt.Scan(&namauniv)
			Array[jmluniv].Univ = namauniv
			arrLomba.Pemrograman[jmlLomba].Univ = namauniv
			jmluniv += 1
			fmt.Println("masukkan nilai skor : ")
			fmt.Scan(&skor)
			Array[jmlskor].Skor = skor
			arrLomba.Pemrograman[jmlLomba].Skor = skor
			jmlskor += 1
			jmlLomba++
			jmlJenis2++
		}
		if jenis == 3 {
			...
			fmt.Scan(&namatim)
			Array[jmltim].Nama = namatim
			arrLomba.Animasi[jmlLomba].Nama = namatim
			jmltim += 1
			JumlahTim += 1
			fmt.Println("masukkan asal universitas : ")
			fmt.Scan(&namauniv)
			Array[jmluniv].Univ = namauniv
			arrLomba.Animasi[jmlLomba].Univ = namauniv
			jmluniv += 1
			fmt.Println("masukkan nilai skor : ")
			fmt.Scan(&skor)
			Array[jmlskor].Skor = skor
			arrLomba.Animasi[jmlLomba].Skor = skor
			jmlskor += 1
			jmlLomba++
			jmlJenis3++
		}
